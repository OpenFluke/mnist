...
func evolveModel(originalBP *phase.Phase, samples []Sample, checkpoints []map[int]map[string]interface{}, copyID int) ModelResult {
	...
	bestCompositeScore := computeCompositeScore(bestExactAcc, computeClosenessQuality(bestClosenessBins), bestApproxScore)
	neuronsAdded := 0
	maxAttempts := 10 // Increased from 5 to explore more

	// Channel for results
	attemptChan := make(chan struct {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		go func() {
			...
			tempBP := bp.Copy()
			tempNeuronsAdded := 0
			numToAdd := rand.Intn(100) + 1 // Wider range: 1-100
			neuronType := selectRandomNeuronType()
			activation := selectRandomActivation()

			// Add neurons with variety
			for i := 0; i < numToAdd; i++ {
				newN := tempBP.AddNeuronFromPreOutputs(neuronType, activation, 1, 50)
				if newN == nil {
					continue
				}
				tempBP.AddNewNeuronToOutput(newN.ID)
				tempNeuronsAdded++
			}

			// Evaluate the new model
			exactAcc, closenessBins, approxScore := evaluateModelWithCheckpoints(tempBP, samples, checkpoints)
			compositeScore := computeCompositeScore(exactAcc, computeClosenessQuality(closenessBins), approxScore)
			...
			attemptChan <- struct {
				...
				BP             *phase.Phase
				NeuronsAdded   int
			}{compositeScore, exactAcc, closenessBins, approxScore, tempBP, tempNeuronsAdded}
		}()
	}

	// Close channel when all attempts are done
	go func() {
		wg.Wait()
		close(attemptChan)
	}()
	...
	// Process results
	const epsilon = 0.0001 // Small threshold for improvement
	for result := range attemptChan {
		if result.CompositeScore > bestCompositeScore+epsilon {
			bestCompositeScore = result.CompositeScore
...
// computeCompositeScore combines metrics into a single score
func computeCompositeScore(exactAcc, closenessQuality, approxScore float64) float64 {
	return 0.4*exactAcc + 0.3*closenessQuality + 0.3*approxScore // Adjustable weights
}

// selectRandomNeuronType picks a random neuron type
func selectRandomNeuronType() string {
	types := []string{"dense", "lstm", "cnn"}
	return types[rand.Intn(len(types))]
}

// selectRandomActivation picks a random activation function
func selectRandomActivation() string {

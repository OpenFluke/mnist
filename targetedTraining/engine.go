...
const (
	...
	mutationRate        = 0.3
	topPercentage       = 0.05 // Top 5% of models to improve
	...
	enableTargetedMutation = false

	adjustmentMin  = -0.05 // Minimum adjustment value
	adjustmentMax  = 0.05  // Maximum adjustment value
	adjustmentStep = 0.01  // Step size for adjustments
)
...
func trainTesting(trainX, trainY, testX, testY *mat.Dense) {
	...
	for gen < numGenerations {
		...
		// **Improvement Step: Adjust the top 5% of models using a spectrum of adjustments**
		...
		// Replace the top models in the population with their improved versions
...
// ============================================================================
// New Functions for Spectrum Adjustments
// ============================================================================

// generateAdjustments creates a list of adjustment values from min to max with the given step
func generateAdjustments(min, max, step float64) []float64 {
	var adjustments []float64
	for adj := min; adj <= max; adj += step {
	...
	for _, task := range tasks {
		...
		go func(task AdjustmentTask) {
			...
			if task.adjustWeights {
				candidate.AdjustAllWeights(task.adjValue)
			}
...
func evaluateFitness(bp *phase.Phase, testX, testY *mat.Dense) float64 {
	exactAcc, closeAcc, proximityScore := evaluateAccuracy(bp, testX, testY)
	return (0.5 * exactAcc) + (0.3 * closeAcc) + (0.2 * proximityScore / 100)
}
...
func selectTopPerformers(pop []*phase.Phase, exactAccs, closeAccs, proximityScores []float64, frac float64) []*phase.Phase {
	...
	if numSelect < 1 {
		numSelect = 1 // Ensure at least one model is selected
	}
...
func createNextGeneration(parents []*phase.Phase, popSize int, mRate float64, cLevel int, overallBestModel *phase.Phase, knownSamples map[int]bool, testX, testY *mat.Dense) []*phase.Phase {
	...
	out[0] = overallBestModel.Copy() // Preserve the best model

	// Check if targeted mutation is disabled
	if !enableTargetedMutation {
		// Use regular mutation for the entire population (except the best model)
		for i := 1; i < popSize; i++ {
	...
	// Gather unknown samples for targeted mutation
	...
	// Apply targeted mutation to the remaining portion
	for i := targetedStart; i < popSize; i++ {
		...
		// Update knownSamples based on the bestChild’s performance
...
func performTargetedMutation(bp *phase.Phase, sampleIndices []int, testX, testY *mat.Dense, mRate float64, cLevel int) *phase.Phase {
	...
	for attempt := 0; attempt < maxAttempts && len(currentBatch) > 0; attempt++ {
		...
		// Generate mutations with specific weight and bias adjustments
		for i := 0; i < numGoroutines; i++ {
			wg.Add(1)
			go func() {
				defer wg.Done()
				candidate := bestModel.Copy()
				// Apply small random normal perturbations to all weights and biases
				candidate.AdjustAllWeights(rand.NormFloat64() * 0.01)
				candidate.AdjustAllBiases(rand.NormFloat64() * 0.01)
				score := 0.0

...
func main() {
	...
	if _, err := os.Stat(modelPath); err == nil {
		...
		// Suppose you want each neuron in layer 1 (the {16,16} layer) to hold a mini 1×1 -> 1×1 sub-network:
		...
		/*nn.SetLayerDimensionShared(
			...
			subLayerSizes,     // e.g. { {1,1}, {2,2}, {1,1} }
			subActivations,    // e.g. {"relu", "relu", "linear"}
			subFullyConnected, // e.g. {true, true, true}
		)*/

		nn.Train(inputs, targets, 5, 0.01)
		//nn.TrainBatch(inputs, targets, 5, 0.001, 32)
